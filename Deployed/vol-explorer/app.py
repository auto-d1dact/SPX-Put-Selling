...
def make_dollar_std_plot(hidden, ticker, graph_toggles,
    ...
    if hidden == 'loaded':
        ...
        y_lb = min(hist_data.open.min(), hist_data.close.min())
        y_ub = max(hist_data.open.max(), hist_data.close.max())
...
# Update Daily Dollar std
@app.callback(Output('daily_dollar_std', 'children'),
              [Input('filtered_container', 'hidden'),
               Input('ticker_dropdown', 'value')])
def update_daily_dollar_std(hidden, ticker):

    if hidden == 'loaded':
        last_price = '{} Daily Dollar Std.: ${}'.format(ticker, str(round(hist_data.tail(1).daily_dollar_vol[0], 2)))
        return last_price
